...
\label{sec:org23ddbeb}
Una \textbf{sucesión} en un conjunto \(X\) es una aplicación \(s: \mathbb{N}
  \rightarrow X\;;\; i \mapsto s(i) = a_i\). Cuando se tiene una aplicación \(f: X \rightarrow  Y\) y una sucesión \(s: \mathbb{N} \rightarrow X\), definimos la sucesión \(s': \mathbb{N} \rightarrow Y\) a la composición \(s' = f \circ s\) de modo que \(s'(i) = f\) o \(s(i)
...
\label{sec:orga4f76f6}
Sea una aplicación \(f:(X,T) \rightarrow  Y\). La familia de conjuntos de \(Y\) tales que sus imágenes inversas son abiertos de \(T\) constituyen una
...
\label{sec:org9a737ae}
Un espacio topológico \((X,T)\) es \textbf{conexo} cuando no existen en él dos abiertos
