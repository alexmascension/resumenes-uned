...
(x-r, x+r) \}_{0<r \in \mathbb{Q}}x  \) es un sistema de entornos numerable.
...
espacio topológico \( (Y, T') \), se llama *topología inducida* o *topología
inicial* por \( f \) en \(
...
*Propiedad universal de la topología inducida (varias aplicaciones)*. 
Sea \( (X,T) \) un espacio
