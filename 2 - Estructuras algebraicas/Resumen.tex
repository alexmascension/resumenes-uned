...
\label{sec:org2ad0cc3}
\subsection{Grupos}
\label{sec:orgd3ad721}
...
\lema{2.20.3} Sean \(H,K \subset_N G\) tales que \(H \cap K = \{ 1 \}\). Entonces \(HK \simeq H \times K\).
...
\label{sec:org20298dd}
\subsection{Teorema de la Estructura}
\label{sec:org9549b27}
...
\mathbb{Z}/m_r \mathbb{Z} \times \{0\} \times \cdots \times \{0\}\) y \(T(G) =\mathbb{Z} \times \cdots \times \mathbb{Z}\)

\obs{7.4} Si \(\phi: G_1 \rightarrow G_2\) es un isomorfismo entre grupos abelianos, las aplicaciones \(\phi|_{T(G_1)}:
T(G_1) \rightarrow T(G_2): x \mapsto \phi(x)\) y \(\psi: G_1/T(G_1) \rightarrow G_2/T(G_2): x + T(G_1) \mapsto \phi(x) + T(G_2)\) también son isomorfismos

\lema{7.5} Sea \(\{ x_1, \cdots, x_k \}\) un sistema generador minimal de \(G\). Si existen enteros no nulos \(m_1, \cdots, m_k\), tales que \(\prod
x_i^{m_i} = 1\), entonces \(G\) tiene torsión.

\lema{7.6} Sea \(G\) abeliano, sin torsión y finitamente generado. Si \(n\)
