...
\label{sec:org920af1c}
\subsection{Grupos}
\label{sec:org3458fa2}
...
\obs{2.2.4/2.2.} Si \(H\) es un subgrupo de \(G\), y \([G:H] = 2\), \(H\) es subgrupo normal de \(G\). Asimismo, los subgrupos \(\{ 1_G \}, G\) son 
normales.
...
\propo{2.2.13} Si \(N \subset_N G\), \(H \subset K \subset G\), y \(H \subset_N K\), entonces \(NH \subset_N NK\).
