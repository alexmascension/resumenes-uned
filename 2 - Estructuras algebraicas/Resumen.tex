...
\label{sec:org47c3d8b}
\subsection{Grupos}
\label{sec:org5f7d641}
...
$\backslash$(
\begin{tikzpicture}[]
\node (topleft) at (0,1*4) {\(A\)};
\node (botleft) at (0,0) {\(B\)};
\node (botright) at (2*4,0) {\(C\)};
\node (topright) at (2*4,1*4) {\(D\)};

\draw[->,thick] (topleft) -- node[above] {\(f_{ad}\)} (topright);
\draw[->,thick] (topleft) -- node[left] {\(f_{ab}\)} (botleft);
\draw[->,thick] (botleft) -- node[below] {\(f_{bc}\)} (botright);
\draw[->,thick] (botright) -- node[right] {\(f_{cd}\)} (topright);
\end{tikzpicture}
$\backslash$)
\end{document}