...
biyectivas. Es importante que pese a existir una biyeccíón entre \( Hx \) y \(
xH \), *no siempre \( Hx \) y \( xH \) son iguales*.

\propo{1.12.3} La aplicación entre conjuntos cocientes \( G/R_H \rightarrow G/R^H: Hx \rightarrow x^{-1}H
\) es biyectiva.
...
[G:H] \) es el número de elementos de \( G/R_H \).

\propo{1.12.8 (Tma de Lagrange)} Sea \( H \subset G
\) un subgrupo. Se cumple que si \( G \) es finito, entonces \( o(H) \) es
