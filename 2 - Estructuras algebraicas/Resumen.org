...
\obs{2.2.4/2.2.} Si \( H \) es un subgrupo de \( G \), y \( [G:H] = 2 \), \( H
...
\propo{2.2.13} Si \( N \subset_N G
\), \( H \subset K \subset G
\), y \( H \subset_N K \), entonces \( NH \subset_N NK \).
...
\propo{2.2.17 [Teorema de Poincaré]} Si \( G \) posee un subgrupo de índice
