...
\) y \( T(G) =\mathbb{Z} \times \cdots \times \mathbb{Z}  \)

\obs{7.4} Si \( \phi: G_1 \rightarrow G_2
\) es un isomorfismo entre grupos abelianos, las aplicaciones \( \phi|_{T(G_1)}:
T(G_1) \rightarrow T(G_2): x \mapsto \phi(x)
\) y \( \psi: G_1/T(G_1) \rightarrow G_2/T(G_2): x + T(G_1) \mapsto \phi(x) + T(G_2)
\) también son isomorfismos

\lema{7.5} Sea \( \{ x_1, \cdots, x_k \} \) un sistema generador minimal de \( G
