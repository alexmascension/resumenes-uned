...
Se demuestra que \( R(S) \simeq \mathbb{Z}^k, k \le n
\), y \( \beta(G) = n - k \).
...
*PASO 1* Dados \( S=\{ x_1, \cdots, x_n \} \) y \( R = \{ r_1, \cdots, r_k \}\), escribimos la
matriz M(S,R)
\[   M(S,R) =  \begin{bmatrix}
a_{11} &  \cdots  &  a_{1n} \\ 
\vdots
...
Si b_{1l} no divide a todos los \( a_{11}, b_{12}, \cdots, b_{1n} \), repetimos el
...
\end{bmatrix}
\]
*PASO 2* Dividimos \( a_{i1} = a_{11}q_{i} + c_{i1} \). Si \( c_{i1} = 0 \) para
...
0 &  b_{k2} &  \cdots  &  b_{kn} \\ 
\end{bmatrix}  \]
*PASO 3* Si \( a_{11} \) no divide a algún \( b_{ij} \), hacemos \( r_1 = r_1 + r_i \)
 y repetimos el paso 1 hasta que todos los elementos \( b \) sean divisibles por
\( a_{11} \).

*PASO 4/5* Sometemos a la submatriz de elementos \( b \) el mismo procedimiento que
